...
func TestSetHealthHttpError(t *testing.T) {
	...
	err := setHealth(t.Context(), *server.Client(), podInfo, "127.0.0.1",
		appObservabilityConf{HealthPorts: []string{"0"}, HealthEndpoints: []string{"/health"}})
	require.Error(t, err)
}
...
func TestSetHealthStatusOK(t *testing.T) {
	...
	err = setHealth(t.Context(), *server.Client(), podInfo, host,
		appObservabilityConf{HealthPorts: []string{portStr}, HealthEndpoints: []string{"/health"}})
	require.NoError(t, err)
...
func TestSetHealthStatusAlternative(t *testing.T) {
	...
	err = setHealth(t.Context(), *server.Client(), podInfo, host,
		appObservabilityConf{HealthPorts: []string{portStr}, HealthEndpoints: []string{"observe/live", "q/live"}})
	require.NoError(t, err)
...
`
	...
	err = setMetrics(t.Context(), *server.Client(), podInfo, host,
		appObservabilityConf{MetricsPorts: []string{portStr}, MetricsEndpoints: []string{"/metrics"}})
	require.NoError(t, err)
...
`
	...
	err = setMetrics(t.Context(), *server.Client(), podInfo, host,
		appObservabilityConf{MetricsPorts: []string{portStr}, MetricsEndpoints: []string{"metrics", "q/metrics", "actuator/prometheus"}})
	require.NoError(t, err)
...
`
	...
	err = setMetrics(t.Context(), *server.Client(), podInfo, host,
		appObservabilityConf{MetricsPorts: []string{"1234"}, MetricsEndpoints: []string{"/metrics"}})
	require.Error(t, err)
	assert.Equal(t, "no valid metrics endpoint found", err.Error())

	// This call points to a non existing port and an existing port, it must succeed
	err = setMetrics(t.Context(), *server.Client(), podInfo, host,
		appObservabilityConf{MetricsPorts: []string{"1234", portStr}, MetricsEndpoints: []string{"/metrics"}})
	require.NoError(t, err)
}
...
func TestHealthAlternativePort(t *testing.T) {
	...
	// This call points to a non existing port, it must fail
	err = setHealth(t.Context(), *server.Client(), podInfo, host,
		appObservabilityConf{HealthPorts: []string{"1234"}, HealthEndpoints: []string{"q/live"}})
	require.Error(t, err)
	assert.Equal(t, "no valid health endpoint found", err.Error())

	// This call points to a non existing port and an existing port, it must succeed
	err = setHealth(t.Context(), *server.Client(), podInfo, host,
		appObservabilityConf{HealthPorts: []string{"1234", portStr}, HealthEndpoints: []string{"q/live"}})
	require.NoError(t, err)
}
